...
public class DesktopTamilYearlyWebStoriesSiteMapValidation extends WebDriverActions {
	...
	@Test
	public void N18_TM_WEB_YEARLY() throws IOException {
		ArrayList<String> smDailyList = new ArrayList<String>();
		String url = fileUtility.readDataFromPropertyFile("tamilYearlyWebStoriesSiteMapUrl");
		Logs.info(getClass(),"Tamil xml Url is : " +url);
		try {
			try {
	...
	public void checkLinksSiteMap(String links,String parentLink)  {
		try {
			...
			if (responseCode>200) {
				...
				assrt.fail("Status code is "+responseCode +" URL has issue in "+links+ " Parent Url : "+parentLink);
			}
			else {
