...
public class DesktopKannadaTodaySiteMapValidation extends WebDriverActions {
	...
	@Test
	public void N18_KA_TDAY() throws IOException {
		ArrayList<String> smDailyList = new ArrayList<String>();
		String url = fileUtility.readDataFromPropertyFile("kannadaTodaySiteMapUrl");
		Logs.info(getClass(),"Kannada xml Url is : " +url);
		try {
			try {
	...
	public void checkLinksSiteMap(String links,String parentLink)  {
		try {
			...
			if (responseCode>200) {
				Logs.error(getClass(), responseCode + "--------------- "  +links+ " Its Parent Url : "+parentLink);
