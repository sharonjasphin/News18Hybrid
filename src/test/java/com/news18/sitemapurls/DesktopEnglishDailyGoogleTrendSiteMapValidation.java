...
public class DesktopEnglishDailyGoogleTrendSiteMapValidation extends WebDriverActions {
	...
	public void verifyGoogleTrends() throws Throwable
	{
		...
		//		String status="";
		String url = fileUtility.readDataFromPropertyFile("googleTrendUrl");
		Logs.info(getClass(),"Daily Site Map link is : " +url);

		try {
			String siteMapUrl = getCurrentPageUrl();
			synonymsList=dbGetGoogleTrendsSynonyms();
			smDailyList=getSiteMapLinks(siteMapUrl);
			Logs.info(getClass(), smDailyList);
			...
			//checking for Google Trends Words
			...
			for(String j:textList)
			{
				...
				if(urlList.size()==0)
				{	
					str=("   KeyWord not found in SiteMap URL  "+"view-source:"+siteMapUrl );
					//					status="NO";
