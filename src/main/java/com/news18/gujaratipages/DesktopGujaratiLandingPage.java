...
public class DesktopGujaratiLandingPage {
	...
	/* header Links*/
	@FindBy(xpath ="//div[contains(@class,'nhhdr')]/div[contains(@class,'_lnlivetv')]/div[not (contains(@class,'_languagebox'))]//a | //div[contains(@class,'nhtranding')]/a" )
	...
	public void verifyChangeLanguage() {
		...
		if (changeLanguageList.size()>0) {
			for (int i = 1; i < changeLanguageList.size(); i++) {
				webDriverActions.softHighlightElement(changeLanguageBtn);
				webDriverActions.softMouseHover(changeLanguageBtn);
				webDriverActions.softHighlightElement(changeLanguageList.get(i));
				webDriverActions.softMouseHover(changeLanguageList.get(i));
	...
	public void headerValidation()
	{
		...
		softAssert.assertTrue(headerList.size()>0,"Unable To Display the Element"+ webDriverActions.getCurrentPageUrl());
		for (int i = 0; i < headerList.size(); i++) {
			webDriverActions.softHighlightElement(headerList.get(i));
			webDriverActions.softMouseHover(headerList.get(i));
			String pageUrl = webDriverActions.getAttribute("href", headerList.get(i));
			String title = webDriverActions.getAttribute("target", headerList.get(i));
			webDriverActions.softClickJS(headerList.get(i));
	...
	public void verfyLineOneSection(String server)
	{
		...
		{
			String pageUrl = webDriverActions.getAttribute("href", lineOneSectionELementLnk.get(i));
	...
	public void verfyLineTwoSection(String server)
	{
		...
		for (int i = 0; i < lineTwoSectionsLnk.size(); i++)
		{
			...
			String pageUrl = webDriverActions.getAttribute("href", lineTwoSectionsLnk.get(i));
	...
	public void verifyLineOneSectionChooseYourDistrict(String server)
	{
		...
		for(int i=0;i<chooseYourDistrictEleList.size();i++)
		{
			...
			String pageUrl=webDriverActions.getAttribute("href", chooseYourDistrictEleList.get(i));
	...
	/**
	 * @author SanjeebKumarPati
	 * This method used to verify footer  in desktop Landing page.
	 */
	public void verifyFooterSection(WebElement footerSection,List<WebElement> footerEleList)
	{
		String mainUrl = webDriverActions.getCurrentPageUrl();
		...
		softAssert.assertTrue(footerEleList.size()>0,"Unable To Display the Element"+ webDriverActions.getCurrentPageUrl());
		for(int i=0;i<footerEleList.size();i++)
		{
			webDriverActions.softHighlightElement(footerEleList.get(i));
			webDriverActions.softMouseHover(footerEleList.get(i));
			String pageUrl=webDriverActions.getAttribute("href", footerEleList.get(i));
			String title = webDriverActions.getAttribute("target", footerEleList.get(i));
			webDriverActions.softClickJS(footerEleList.get(i));
			if(title.contains("_blank"))
			{
				webDriverActions.switchToNewWindow();
			}
			...
			Logs.info(getClass(), pageUrl);
			Logs.info(getClass(), currentUrl);
			boolean status=genericUtility.compareUrlSub(currentUrl, pageUrl);
			softAssert.assertTrue(response==200 , "Possibly Url is Invalid "+currentUrl);
			softAssert.assertTrue(status, "Possibly not Navigate to same Page "+currentUrl);
			if(title.contains("_blank"))
			{
		...
		softAssert.assertAll("Footer Section Validation Failed");
	}
	...
	{
		verifyFooterSection(footerLiveTVSection, liveTvEleList);
	}
	...
	{
		verifyFooterSection(footerBivagSection, footerBivagEleList);
	}
	...
	{
		verifyFooterSection(footerAboutUSSection, footerAboutUSEleList);
	}
	...
	{
		verifyFooterSection(footerLatestNewsSection, footerLatestNewsEleList);
	}
	...
	public void verifyFooterNews18GroupSitesSection()
	{
		...
		webDriverActions.softMouseHover(footerGroupSitesSection);
		...
		{
			webDriverActions.softHighlightElement(footerGroupSitesEleList.get(i));
			webDriverActions.softMouseHover(footerGroupSitesEleList.get(i));
			String pageUrl=webDriverActions.getAttribute("href", footerGroupSitesEleList.get(i));
		...
		{	
			verifyFooterSection(footerSocialShareSection, footerSocialShareEleList);
		}
