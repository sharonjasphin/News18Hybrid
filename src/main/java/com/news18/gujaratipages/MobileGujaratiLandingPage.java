...
//			webDriverActions.softClickJS(kannadaSectionNextBtn);
	...
	public void districtIconCitiesNavigations(String serverType)
	{
		...
		for (int i = 0; i < districtIconCities.size(); i++)
		{
			...
			String pageUrl= webDriverActions.getAttribute("href", districtIconCities.get(i));
	...
	public void verfyLineOneSectionAndSearchIcon()
	{
		try {
			...
			softAssert.assertTrue(lineOneSectionELementLnk.size()>0, "possibly line One Element Is not Visible");
	...
	public void verifySocialSites()
	{
		...
		webDriverActions.softHighlightElement(socialShareSection);
		...
		for(int i=0;i<socialShareSectionEle.size();i++)
		{
			...
			String pageUrl = webDriverActions.getAttribute("href",socialShareSectionEle.get(i));
			Logs.info(getClass(), pageUrl);
	...
	public void verifyHamburgerLanguageSection(String serverType) 
	{
		...
		webDriverActions.waitForPageToLoad();
		webDriverActions.softHighlightElement(hambergerIcon);
		...
		for (int i = 0; i < hamburgerLangLnks.size(); i++) {
			webDriverActions.softHighlightElement(hamburgerLangLnks.get(i));
			webDriverActions.softMouseHover(hamburgerLangLnks.get(i));
			String pageUrl = webDriverActions.getAttribute("href", hamburgerLangLnks.get(i));
			Logs.info(getClass(), pageUrl);
	...
	public void verifyHamburgerSectionNavigation(String serverType)
	{
		...
		for (int i = 1; i < hamburgerHomeSectionLnks.size(); i++) {
			...
			String pageUrl = webDriverActions.getAttribute("href", hamburgerHomeSectionLnks.get(i));
			Logs.info(getClass(), pageUrl);
	...
	{
		webDriverActions.softHighlightElement(footerStickySectionEle.get(i));
		String pageUrl=webDriverActions.getAttribute("href", footerStickySectionEle.get(i));
		...
		Logs.info(getClass(), currentUrl);
		boolean status=genericUtility.compareUrlSub(pageUrl, currentUrl);
		softAssert.assertTrue(response==200 , "Possibly Url is Invalid "+currentUrl + "Status Code is "+response);
		softAssert.assertTrue(status, "Possibly not Navigate to same Page "+currentUrl);
		webDriverActions.navigateBack();
		webDriverActions.continuousScrollTillElement(footerStickySection,20);
	}
