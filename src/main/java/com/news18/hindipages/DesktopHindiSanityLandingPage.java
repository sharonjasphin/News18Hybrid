...
public class DesktopHindiSanityLandingPage {
	...
	public void verfyLiveBlog(String serverType, List<WebElement> image,WebElement breadcrum)
	{
		...
		if(webDriverActions.isElementDisplayed(image.get(0)))
		{
			...
			webDriverActions.clickJS(image.get(0));
			...
			softAssert.assertTrue(webDriverActions.softIsElementDisplayed(breadcrum) , "Unable to Display Element in"+liveBlogUrl);
		}
		else
		{
	...
	public void verifyTrendingAndRecommendedStoriesNavigation(String serverType)
	{
		...
		webDriverActions.softHighlightElement(upcomingTab);
	...
	public void verifyUpcomingMobileStoriesNavigation(String serverType)
	{
		...
		webDriverActions.softHighlightElement(trendingTab);
	...
	public void verfyFooterNewsSitesSection()
	{
		...
		for(int i=0;i<1;i++)
		{
			...
			String actual = webDriverActions.getCurrentPageUrl();
			softAssert.assertTrue(webDriverActions.softIsElementDisplayed(townHallAndTopLearningIcon), "Possibly Element Is Not Visible For This URL "+actual);	
		}
	...
	public void clickOnFirstArticleHomePage(String serverType) {
		...
		webDriverActions.waitForPageToLoad();
		webDriverActions.waitForElement(1000);
		if (webDriverActions.softIsElementDisplayed(homePageArticlesLnk)) {
			...
			softAssrt.assertTrue(webDriverActions.softIsElementDisplayed(articleBreadcrumb, 1), "Possibily element is not visible "+ articleBreadcrumb+" "+currentPage);
		}
		else {
			Logs.error(getClass(),"Possibily element is not visible "+ homePageArticlesLnk+" "+webDriverActions.getCurrentPageUrl());
			softAssrt.fail("Possibily element is not visible "+ homePageArticlesLnk+" "+webDriverActions.getCurrentPageUrl());
		}
	...
	public void clickOnPhotoSection(String serverType)
	{
		...
		webDriverActions.waitForPageToLoad();
		webDriverActions.waitForElement(1000);
		if (webDriverActions.softIsElementDisplayed(photoSectionLineTwoLnk, 1)) {
			...
			Logs.error(getClass(),"Possibily element is not visible "+ photoSectionLineTwoLnk+" "+webDriverActions.getCurrentPageUrl());
			softAssrt.fail("Possibily element is not visible "+ photoSectionLineTwoLnk+" "+webDriverActions.getCurrentPageUrl());
		}
		softAssrt.assertAll("Issue found in Photo section : ");
	}
	...
	public void navigateToVideosSection() {
		...
		if (webDriverActions.softIsElementDisplayed(videosLnk, 1)) {
			...
			if (videosArticleLnks.size()>0) {
				webDriverActions.softClickJS(videosArticleLnks.get(0));
			}
			else {
				Logs.error(getClass(),"Possibily element is not visible "+webDriverActions.getCurrentPageUrl());
				softAssrt.fail("Possibily element is not visible "+webDriverActions.getCurrentPageUrl());
			...
			softAssrt.fail("Possibily element is not visible "+ videosLnk+" "+webDriverActions.getCurrentPageUrl());
		}
		softAssrt.assertAll("Issue found in Photo section : ");
	}
	...
	public void topicsLinks(String serverType) {
		webDriverActions.waitForPageToLoad();
		webDriverActions.waitForElement(1000);
		softAssert.assertTrue(topicslinks.size()>0,"Possibily element is not visible " +webDriverActions.getCurrentPageUrl());
		for (int i = 0; i < 1; i++) {
			webDriverActions.scrollToLast();
	...
	{
		verifyLineOneSection(serverType, lineOneSectionELementLnk, headerTxtLogoLnk);
	}
	 ...
	 */
	public void verifyLineOneSection(String serverType,List<WebElement> eleList,WebElement headerTxtLogoLnk) {
		String actual = "";
		webDriverActions.waitForPageToLoad();
