...
public class AMPHindiSanityLandingPage {
	...
	public AMPHindiSanityLandingPage(WebDriver driver) {
		this.driver = driver;
	...
	@FindBy(xpath ="//div[contains(@class,'brade_crum')] | //div[contains(@class,'br')]  | //ul[contains(@class, 'br')] | //div[contains(@class,'mbl-header')]")
	...
	@FindBy(xpath ="(//ul[contains(@class,'ayodhyaspecial')])[2]//a[1]")
	private WebElement cricketArticleWidgetLnk;

	/*Live blog Headline*/
	@FindBy(xpath ="//ul[contains(@class,'global-storylist')]//child::li//a[contains(@href,'live')]")
	...
	/*Video Lnk present in line one navigation bar of Hindi Home Page*/
	@FindBy(xpath="//ul[contains(@class,'menu-list')]//li//a[contains(@href,'/videos/')]")
	private WebElement videosLnk;

	/*Video Article Link  for news18 Hindi Page*/
	...
	@FindBy(xpath = "//div[contains(@class,'globalhd')]//h2//a[contains(@href,'cricket')]")
	...
	public void verifyFoodArticleNavigation(String serverType) {
		...
		if(foodSectionArticleLst.size()>0)
		{
			...
			{  
				...
				webDriverActions.softWaitForElementVisibility(foodSectionBreadcrum, 3);
				softAssert.assertTrue(webDriverActions.softIsElementDisplayed(foodSectionBreadcrum),"Unable to display Element "+currentPage);
			}
		}
		else
	...
	public void verifyAutoArticleNavigation(String serverType) {
		...
		if(autoArticleLst.size()>0)
		{
			...
			{  
				...
				webDriverActions.softWaitForElementVisibility(articleBreadcrum, 3);
				softAssert.assertTrue(webDriverActions.softIsElementDisplayed(articleBreadcrumb),"Unable to display Element "+currentPage);
				Logs.info(getClass(),"Clicked Latest News articles link");
			}
		}
	...
	public void verifySportsArticleNavigation(String serverType) {
		...
		if(khelArticleLnk.size()>0)
		{
			...
			{  
				...
				webDriverActions.softWaitForElementVisibility(articleBreadcrum, 3);
				webDriverActions.softHighlightElement(articleBreadcrum);
				softAssert.assertTrue(webDriverActions.softIsElementDisplayed(articleBreadcrum),"Unable to display Element "+currentPage);
			}
		}
		else
	...
	public void verifySuperHitGalleryArticleLnk(String serverType) {
		...
		if(superHitGalleryArticleLink.size()>0)
		{
			...
			{  
				...
				webDriverActions.softWaitForElementVisibility(superHitGalleryBreadcrum, 3);
				softAssert.assertTrue(webDriverActions.softIsElementDisplayed(superHitGalleryBreadcrum),"Unable to display Element "+currentPage);
			}
		}
		else
	...
	public void verifyArticleUrl() {
		...
		webDriverActions.waitForPageToLoad();
		for (int i = 0; i < tazaSamacharArticleLnk.size(); i++) {
			String ampUrl = webDriverActions.getAttribute("href", tazaSamacharArticleLnk.get(i));
			softAssert.assertTrue(ampUrl.contains("/amp/"), "Article Link does not have AMP Url "+ampUrl);
		}
	}
	...
	public ArrayList<String> priorityPanelArticles(String serverType) {
		...
		softAssert.assertAll("Issue in priority Panel Article :");
		return list;
	}
	...
	public ArrayList<String> verifyCricketNextArticleLnk(String serverType)
	{
		...
		webDriverActions.waitForPageToLoad();
		webDriverActions.waitForElement(1000);
		softAssert.assertTrue(webDriverActions.softIsElementDisplayed(cricketArticleWidgetLnk, 1));
		String cricketUrl = webDriverActions.getAttribute("href", cricketArticleWidgetLnk);
		if (serverType.contains("preProd") && !cricketUrl.contains("//beta")) {
			String betaUrl = genericUtility.replaceInUrl(cricketUrl);
			cricketUrl=betaUrl;
		}
		list.add(cricketUrl);
		softAssert.assertAll("Issue in CricketNext Articles : ");
		return list;
	}
	...
	public ArrayList<String> homePageLiveBlogArticle(String serverType) {
		...
		webDriverActions.waitForPageToLoad();
		webDriverActions.waitForElement(1000);
		if(webDriverActions.softIsElementDisplayed(liveBlogArticleLnk, 1))
		{
