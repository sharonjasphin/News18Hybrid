...
public class WebDriverActions extends DriverUtility{
	...
	public boolean softIsElementDisplayed(WebElement element) {
		try {
			softWaitForElementVisibility(element, 5);
			boolean status = element.isDisplayed();
			Logs.info(getClass(), "Element displayed "+element);
			return status;
		} catch (Exception e) {
	 ...
	 * @return String
	 */
	public String getTextFromElementJS(WebElement element) {
		String status="";
		try {
			status= (String) js.executeScript("return arguments[0].firstChild.textContent;", element);
			Logs.info(getClass(), "Getting text from "+element);
		} catch (Exception e) {
			Logs.error(getClass(), "not able to get text from"+element);
		}
		return status;
	}
}
