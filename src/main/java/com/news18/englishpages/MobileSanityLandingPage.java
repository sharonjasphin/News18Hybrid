...
public class MobileSanityLandingPage {
	...
	@FindBy(xpath = "//div[contains(@class,'cn_widget')]/a[not (contains(text(),'MORE FROM NEWS18 CRICKETNEXT'))]")
	...
	@FindBy(xpath ="(//div[contains(@class,'BreadCum')]/span)[1]|//div[contains(@class,'bradcurm')]")
	...
	@FindBy(xpath ="//div[contains(@class,'HamburgerBox')]")
	...
	public void verifyPriorityArticleNavigation(String serverType , List<WebElement> artList , WebElement element)
	{
		...
		for(int i=0; i<artList.size() ;i++)
		{
			...
			if (currentPage.equalsIgnoreCase(actual)) {
				webDriverActions.navigateBack();
			}
			else {
				webDriverActions.navigateBack();
	...
	public void verifyArticlesNavigation(WebElement section ,List<WebElement> artList ,String serverType)
	{
		...
		for(int i=0; i<1 ;i++)
		{
			...
			softAssert.assertTrue(webDriverActions.softIsElementDisplayed(articleBreadcrumb) , "Unable to Display Element "+moviesArticleBreadcrumb+" after Clicking on "+actualUrl);
		}
	...
	public void verifyVideoWallArticleNavigation(String serverType)
	{
		...
		softAssert.assertTrue(webDriverActions.softIsElementDisplayed(videoWallArticleBreadcrumb, 2),"Unable to Display Element "+videoWallArticleBreadcrumb+ " after Clicking on "+currentPage);
	...
	{
		verifyArticleNavigation( serverType,videosSectiontArticleLnk, articleBreadcrumb);
	}
	...
	public void verfyLineOneSection(String server)
	{
		try {
			...
			for (int i = 1; i < lineOneSectionELementLnk.size(); i++)
			{
				...
				softAssert.assertTrue(webDriverActions.softIsElementDisplayed(lineOneSectionELementLnk.get(i)), "possibly line One Element Is not Visible"+pageUrl);
...
//		
		...
		webDriverActions.waitForElement(1000);
		webDriverActions.softClickJS(hamburgerrIcon);
		if (hamburgerHeaderIcon.size()>0) {
			for (int i = 0; i < hamburgerHeaderIcon.size(); i++)
			{
				String pageUrl = webDriverActions.getAttribute("href",hamburgerHeaderIcon.get(i));
				Logs.info(getClass(), pageUrl);	
				String title = webDriverActions.getAttribute("target", hamburgerHeaderIcon.get(i));
				webDriverActions.softHighlightElement(hamburgerHeaderIcon.get(i));
			...
			Logs.error(getClass(),"Possibly element is not visible or size is : "+ hamburgerHeaderIcon.size()+" in Url " +mainUrl);
		}
	...
	public void topicsLinks(String serverType) {
		...
		Assert.assertTrue(topicslinks.size()==0," Possibly no Topics links present "+ webDriverActions.getCurrentPageUrl());
		for (int i = 0; i < 1; i++) {
	...
	public void verifyHamburgerSection(List<WebElement> sectionEleList,String server)
	{
		...
		String mainUrl = webDriverActions.getCurrentPageUrl();
		webDriverActions.waitForElement(1000);
		webDriverActions.softMouseHover(hamburgerrIcon);
		webDriverActions.softClickJS(hamburgerrIcon);
		if (sectionEleList.size()>0) {
			for (int i = 0; i < 5; i++)
			{
				String pageUrl = webDriverActions.getAttribute("href",sectionEleList.get(i));
				Logs.info(getClass(), pageUrl);	
				...
				webDriverActions.stopPageLoading();
				int response = genericUtility.checkLinks(currentPage);
				boolean status=genericUtility.manageUrl(pageUrl, currentPage);
				softAssert.assertTrue(response==200 , "Possibly Url is Invalid "+currentPage);
				softAssert.assertTrue(status," Possibly not Navigate to same Page : "+" Actual Url "+pageUrl+ "Navigated Url " +currentPage);				if(title.contains("_blank"))
				{
					webDriverActions.closeDriver();
					webDriverActions.switchToWindow(pageTitle);
					...
					webDriverActions.navigateToUrl(mainUrl);
					webDriverActions.softWaitForElementVisibility(hamburgerrIcon, 2);
					webDriverActions.softMouseHover(hamburgerrIcon);
					webDriverActions.softClickJS(hamburgerrIcon);
				}
			}
			...
			Logs.error(getClass(),"Possibly element is not visible or size is : "+ sectionEleList.size()+" in Url " +mainUrl);
		}
		softAssert.assertAll("Issues Found In Validating Hamburger Menu header Section");
	}
	 ...
	 * This Method is use to verify Hamburger Menu Header Part By clicking on Hamburger Icon it.
	 */
	public void verifyHamburgerHeader()
	{
