...
public class DesktopSanityLandingPage {
	...
	{
		verifyArticleNavigation( serverType,photogalleryArticleLnks, articleBreadcrumb);
	}

	public void verifyMoviesWidgetArticle(String serverType ) {
		verifyArticleNavigation(serverType,moviesWidgetArticlesLnk , articleBreadcrumb);
	}

	public void verifyCricketNextWidgetArticle(String serverType ) {
		verifyArticleNavigation(serverType,cricketNextWidgetArticlesLnk , articleBreadcrumb);
	}

	public void verifyIndiaWidgetArticle(String serverType ) {
		verifyArticleNavigation(serverType,indiaWidgetArticlesLnk , articleBreadcrumb);
	}

	public void verifyBusinessWidgetArticle(String serverType ) {
		verifyArticleNavigation(serverType,businessWidgetArticlesLnk , articleBreadcrumb);
	}
	...
	public void verifyVideosArticleNavigation(String serverType)
	{
		...
		for(int i=0;i<1; i++)
		{
			...
			softAssert.assertTrue(webDriverActions.softIsElementDisplayed(articleBreadcrumb,2) , "Unable To Display Element in "+currentPage);
		}
		softAssert.assertAll("Issues Found in Homepage Body");
	}
	...
	public void verfyLineOneHamburgerMenu(String server)
	{
		...
		for (int i = 0; i < hamburgerSectionsLnk.size(); i++)
		{
			...
			if (response==200) {
				...
				softAssert.assertTrue(status, "Possibly not Navigate to same Page "+expected);

			}
			else {
				softAssert.fail("Possibly Url is Invalid "+expected);

			}
			//			if (expected.equalsIgnoreCase(currentPage)) {
			//				webDriverActions.navigateBack();
			//			}
			//			else {
			//				webDriverActions.navigateBack();
			...
			webDriverActions.softIsElementDisplayed(hamburgerMenuLnk, 1);
			webDriverActions.softClickJS(hamburgerMenuLnk);
		}
	...
	public void verfyLineTwoSection(String server)
	{
		...
		for (int i = 0; i < lineTwoSectionsLnk.size(); i++)
		{
			...
			int response = genericUtility.checkLinks(expected);
			boolean status=genericUtility.compareUrlSub(pageUrl, expected);
			softAssert.assertTrue(response==200 , "Possibly Url is Invalid "+expected);
			softAssert.assertTrue(status, "Possibly not Navigate to same Page "+expected);
			if(title.contains("_blank"))
			{
				webDriverActions.closeDriver();
				webDriverActions.switchToWindow(mainUrl);
			}
			else
			{
				webDriverActions.navigateToUrl(mainUrl);
		...
		softAssert.assertAll("Issues found in HomePage Line Two Section ");
	}
