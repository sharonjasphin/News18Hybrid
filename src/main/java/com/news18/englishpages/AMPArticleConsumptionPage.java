...
public class AMPArticleConsumptionPage {
	...
	@FindBy(xpath = "//div[contains(@class,'story')]/h1/a[not (contains(@href,'/photogallery/')) and not(contains(@href,'/videos/')) and not(contains(@href,'live-')) and not(contains(@href,'livenews'))] | //div[contains(@class,'text')]/a[not (contains(@href,'/photogallery/')) and not(contains(@href,'/videos/')) and not(contains(@href,'live-')) and not(contains(@href,'livenews'))]")
	...
	@FindBy(xpath = "//div[contains(@class,'latest_news')]//a[not (contains(@href,'/photogallery/')) and not(contains(@href,'/videos/')) and not(contains(@href,'live-news-updates')) and not(contains(@href,'live-updates'))and not(contains(@href,'livenews'))]")
	...
	@FindBy(xpath = "//div[contains(@class,'article_content')]//p")
	private List<WebElement> homepageArticleDetailedDescriptionTxt;

	/*Article Detailed Description Header Link in Article Consumption Page*/
	@FindBy(xpath = "//div[contains(@class,'article_content')]//p//a[contains(@href,'news18')]")
	private List<WebElement> homepageArticleDetailedHighlightLnk; 
	...
	@FindBy(xpath ="//div[contains(@class,'ProfileBiography_top_link')] |  //div[contains(@class,'Topic_brade_crum')]")
	...
	/*Trending News Widget Read More Present in */
	...
	public void verifyArticleBreadCrumbNavigation(String serverType , List<WebElement> articleLst , List<WebElement> articleBreadCrumb)
	{
		...
		for(int i=0; i<1; i++) {
			...
			webDriverActions.waitForPageToLoad();
			softAssert.assertTrue(articleBreadCrumb.size()>0, "Possibly Breadcrumb is Not Visible in "+currentPageUrl);
	...
	public void verifyArticleReadMoreDescription(String serverType , List<WebElement> articleLst , WebElement readMoreLnk , WebElement descriptionTxt)
	{
		...
		for(int i=0; i<1; i++) {
			...
			if(webDriverActions.softIsElementDisplayed(readMoreLnk))
			{
				...
				softAssert.assertNotEquals(beforeClick, afterClick , "Possibly Read More Description Link is Not Working "+currentPageUrl);
			}
			if (currentPageUrl.contains(expectedUrl)) {
				webDriverActions.navigateBack();
			}
	...
	public void verifyArticleAgencyNavigation(String serverType , List<WebElement> articleLst , WebElement agencyLnk , WebElement agencyTxt)
	{
		...
		for(int i=0; i<1; i++) {
			...
			softAssert.assertTrue(response==200, "Possibly Url is Invalid "+currentPageUrl+" And Status Code is "+response);
			softAssert.assertTrue(genericUtility.compareUrlSub(actualUrl, expectedUrl), "Possibly not navigate to same page "+actualUrl);
			Assert.assertTrue(webDriverActions.softIsElementDisplayed(agencyLnk), "Possibly Agency Link is Not Visible in "+currentPageUrl);
			...
			softAssert.assertTrue(webDriverActions.softIsElementDisplayed(agencyTxt), "Possibly Page Content is Not Visible "+currentAgencyUrl);
	...
	public void verifyArticleRelatedNewsNavigation(String serverType , List<WebElement> articleLst , List<WebElement> tagsLnk , List<WebElement> relatedNews)
	{
		...
		for(int i=0; i<articleLst.size(); i++) {
			...
			if(tagsLnk.size()>0)
			{
				...
				softAssert.assertTrue(relatedNews.size()>0, "Possibly LHS Related News is Not Visible in "+currentPageUrl);
			...
			if(headerLnk.size()>0) {
				...
				for(int k=0;k<headerLnk.size();k++)
				{
					...
					if(target.contains("blank"))
					{
						...
						softAssert.assertTrue(headerResponse==200, "Possibly Url is Invalid "+currentHeaderUrl+" And Status Code is "+headerResponse);
	...
	public void articleBannerReadMoreLnk(String serverType , List<WebElement> articleLst,WebElement activeMoreLnk ,  WebElement secondReadMoreLnk) {
		...
		for(int i=0; i<1; i++) {
			...
			if(webDriverActions.softIsElementDisplayed(secondReadMoreLnk)) {
				...
				softAssert.assertFalse(webDriverActions.softIsElementDisplayed(activeMoreLnk) , "Possibly Footer Read More Link is Not Working in "+currentPageUrl);
			}
			else
			{
	...
	{
		verifyArticleDescriptionHeaderLinkNavigation(serverType, homepageArticleLnk, homepageArticleDetailedDescriptionTxt, homepageArticleDetailedHighlightLnk);
	}
				...
				if(webDriverActions.softIsElementDisplayed(tagsDiv, 2)) {
					for(int j=0; j<tagsLnks.size(); j++) {
						...
						if(webDriverActions.getCurrentPageUrl().contains("google_vignette") || webDriverActions.getCurrentPageUrl().contains("PWA_ROS_INSTL"))
						{
							...
							webDriverActions.softMouseHover(tagsLnks.get(j));
							webDriverActions.softScrollToElement(tagsLnks.get(j));
							actualTagUrl = webDriverActions.getAttribute("href", tagsLnks.get(j));
							webDriverActions.softClickJS(tagsLnks.get(j));
							webDriverActions.waitForElement(1000);
						}
			...
			if (response==200)
			{
				...
				if(webDriverActions.softIsElementDisplayed(authorBox, 1)) {
					...
					if(webDriverActions.softIsElementDisplayed(authorBoxMoreLnk, 3)) {
						...
						{
							webDriverActions.navigateBack();
							webDriverActions.softMouseHover(authorBoxMoreLnk);
							webDriverActions.softScrollToElement(authorBoxMoreLnk);
							actualMoreUrl = webDriverActions.getAttribute("href", authorBoxMoreLnk);
							webDriverActions.softClickJS(authorBoxMoreLnk);
							webDriverActions.waitForElement(1000);
						}
	...
	//HomePage
	public void verifyHomePageTagsNavigation(String serverType)
	{
		verifyTagsNavigation(serverType, homepageArticleLnk,tagBreadcrumb);
	}
	
	public void verifyHomePageArticleFirstPublishedAndLastUpdatedNavigation(String serverType)
	{
		verifyArticleFirstPublishedAndLastUpdatedNavigation(serverType, homepageArticleLnk);
	}
	
	public void verifyHomePageArticleAboutTheAuthorNavigation(String serverType)
	{
		verifyArticleAboutTheAuthorNavigation(serverType, homepageArticleLnk);
	}
	...
	public void verifyRHSTitleNavigation(String serverType,WebElement title,List<WebElement> categoryPageLinks) {
		...
		webDriverActions.waitForSecond(1000);
		webDriverActions.softWaitForAllElementsVisible(categoryPageLinks,10);
		...
		for (int i = 0; i < 1; i++) {
			...
			{
				webDriverActions.navigateBack();
				webDriverActions.clickJS(categoryPageLinks.get(i));
				webDriverActions.waitForElement(1000);
			}
			...
			if (response==200) {
				...
				{
					webDriverActions.navigateBack();
					webDriverActions.waitForElementVisibility(title, 20);
					webDriverActions.clickJS(title);
					webDriverActions.waitForElement(1000);
				}
		...
		softAssert.assertAll(" Issues found  category section Title ");
	}
	...
	public void verifyRHSArticleNavigation(String serverType,WebElement title,List<WebElement> articlesLinks,List<WebElement> categoryPageLinks) {
		...
		for (int i = 0; i < 1; i++) {
			webDriverActions.softWaitForElementVisibility(categoryPageLinks.get(i), 10);
			webDriverActions.softHighlightElement(categoryPageLinks.get(i));
			webDriverActions.softMouseHover(categoryPageLinks.get(i));
			String actual=webDriverActions.getAttribute("href", categoryPageLinks.get(i));
			webDriverActions.softClickJS(categoryPageLinks.get(i));
			...
			if (response==200) {
				...
				//				webDriverActions.continuousScrollTillAllElement(articlesLinks, 15);
				...
				String categoryCPPageUrl = webDriverActions.getCurrentPageUrl();
				...
				for(int j=0;j<articlesLinks.size();j++) {
					//					webDriverActions.softWaitForElementVisibility(articlesLinks.get(j), 10);
					webDriverActions.waitForElementVisibility(articlesLinks.get(j), 5);
					...
					{
						webDriverActions.navigateBack();
						webDriverActions.softWaitForAllElementsVisible(articlesLinks, 10);
						webDriverActions.softHighlightElement(articlesLinks.get(j));
						webDriverActions.mouseHover(articlesLinks.get(j));
						webDriverActions.softClickJS(articlesLinks.get(j));
					...
					//					webDriverActions.navigateToUrl(categoryCPPageUrl);
					webDriverActions.navigateBack();
					webDriverActions.waitForSecond(2000);
				}
			}
		...
		softAssert.assertAll("Issues found in  category section Articles ");
	}
	...
	public void verifyRHSReadMoreNavigation(String serverType,WebElement title, WebElement morelink,List<WebElement> categoryPageLinks) {
		...
		for (int i = 0; i < 1; i++) {
			...
			if (response==200) {
				...
				{
					webDriverActions.navigateBack();
					webDriverActions.waitForElementVisibility(morelink, 20);
					webDriverActions.softClickJS(morelink);
					webDriverActions.waitForElement(1000);
				}
				String currentCPPage = webDriverActions.getCurrentPageUrl();
		...
		softAssert.assertAll("Issues found in category section Read more ");
	}
