...
public class AMPPhotoArticleConsumptionPage {
	...
	/*Homepage Photogallery Article Links */
	@FindBy(xpath = "//div[contains(@class,'story')]/h1/a[contains(@href,'/photogallery/')] | //div[contains(@class,'text')]//a[contains(@href,'/photogallery/')]")
	...
	public void verifyArticleBreadCrumbNavigation(String serverType , List<WebElement> articleLst , List<WebElement> articleBreadCrumb)
	{
		...
		for(int i=0; i<1; i++) {
			...
			for(int j=0; j<articleBreadCrumb.size(); j++) {
				...
				currentPageUrlCP = webDriverActions.getCurrentPageUrl();
	...
	public void verifyArticlePhotogallerySocialMediaNavigation(String serverType , List<WebElement> articleLst,List< WebElement> articleSocialMedia)
	{
		...
		for(int i=0; i<1; i++) {
			...
			for(int j=0 ; j<articleSocialMedia.size();j++ )
			{
				...
				softAssert.assertTrue(shareResponse==200, "Possibly Url is Invalid "+actualShare+" And Status Code is "+shareResponse);
			}
		}
		softAssert.assertAll("Issue Found in Article Body ");
	}
	...
	/*================Photogallery Homepage Article Consumption Page=============*/
	...
	/**
	 * @author AnupamGupta
	 * This method is used to validate ads present between photo article consumption page.
	 * @param serverType
	 * @param articleLst
	 */
