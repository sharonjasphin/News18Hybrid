...
public class MetaTagContentValidation {
	...
	public void verifyMetatagcontent() {
		...
		if (!(heading.isEmpty())) {
			...
			boolean status=genericUtility.compareTwoText(heading, metaTagTextcontentLnk);
			softAssert.assertTrue(status, " H1 tag content doesnot match with Title tag in view source page "+webDriverActions.getCurrentPageUrl());
		}
		//		else {
		//			Assert.assertTrue(webDriverActions.isElementDisplayed(header1Lnk)," H1 tag content is not Present in view source page "+" "+webDriverActions.getCurrentPageUrl());
		//		}
		//		Logs.info(getClass(),"H1 tag is Present "+ heading);
		...
		//		Assert.assertTrue(webDriverActions.isElementDisplayed(header1Lnk) && webDriverActions.isElementDisplayed(metaTagTextContent)," H1 tag content or meta title tag is not Present in view source page "+" "+webDriverActions.getCurrentPageUrl());
		...
		if (!(heading2.isEmpty())) {
			...
			boolean status1 = genericUtility.compareTwoText(heading2, metaTagdescriptioncontentTxt);
			softAssert.assertTrue(status1, " H2 tag content doesnot match with Description tag in view source page "+webDriverActions.getCurrentPageUrl());
		}
		softAssert.assertAll();
		//		else {
		//			Assert.assertTrue(webDriverActions.isElementDisplayed(headline2Lnk)," H2 tag content is not Present in view source page "+" "+webDriverActions.getCurrentPageUrl());
		//		}
		//		Logs.info(getClass(),"H2 tag is Present "+ heading2);
...
//				heading2 = s2[0];
				if (!(heading2.isEmpty())) {
					Logs.info(getClass(),"H2 tag is Present "+ heading2);
					boolean status=genericUtility.compareTwoText(metaTagdescriptioncontentTxt,heading2);
					softAssert.assertTrue(status, " H2 tag content doesnot match with Description tag in view source page "+webDriverActions.getCurrentPageUrl());
				}
...
//				Assert.assertTrue(webDriverActions.compareTwoStrings(heading2,metaTagdescriptioncontentTxt)," H2 tag content is not same in view source page meta description tag content "+" "+webDriverActions.getCurrentPageUrl());
	...
	public void verifyGujaratiMetatagcontent() {
		...
		if (!(heading.isEmpty())) {
			Logs.info(getClass(),"H1 tag is Present "+ heading);
			boolean status=genericUtility.compareTwoText(heading2, metaTagdescriptioncontentTxt);
			softAssert.assertTrue(status, " H2 tag content doesnot match with Description tag in view source page "+webDriverActions.getCurrentPageUrl());
		}
		//		Logs.info(getClass(),"H2 tag is Present "+ metaTagdescriptioncontentTxt);
		//		Assert.assertTrue(webDriverActions.compareTwoStrings(heading2,metaTagdescriptioncontentTxt)," H2 tag content is not same in view source page meta description tag content "+" "+webDriverActions.getCurrentPageUrl());
