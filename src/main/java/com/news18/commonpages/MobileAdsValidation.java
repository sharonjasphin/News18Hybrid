...
public class MobileAdsValidation {
	...
	/*readmore present in page */
	@FindBy(xpath="//div[contains(@class,'more_content') or contains (@class,'read-more')]/a")
	private  WebElement readmoreAMPAds;
	 ...
	 * This method used to validate Advertisements present in mobile web 
	 */
	public void verifyArticleAMPAds(WebElement readMore,int scroll,int wait){
		int count=0;
		webDriverActions.waitForPageToLoad();
		webDriverActions.waitForElement(1000);
		String url = webDriverActions.getCurrentPageUrl();
		webDriverActions.mobileScroll(2);

		if(webDriverActions.softIsElementDisplayed(readMore, 4)) {
			webDriverActions.softMouseHover(readMore);
			webDriverActions.softClickJS(readMore);
			webDriverActions.scrollToPoint(0, 100);
			webDriverActions.mobileScroll(3);
		}
	...
	/**
	 * @author AnupamGupta
	 * This method used to validate Advertisements present in odia article page mobile web 
	 */
	public void verifyOdiaArticleAMPAds(int scroll,int wait){
		verifyArticleAMPAds(readmoreAMPAds, scroll, wait);
	}
}