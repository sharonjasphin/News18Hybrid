...
public class DesktopAdsValidation {
	...
	/*List of advertisement present in assam page */
	@FindBy(xpath="//iframe[(@aria-label='Advertisement') and (not(@width<'2'))] | //div[contains(@class,'inner-ad')]//iframe[contains(@class,'jads')]")
	private  List<WebElement> assamDesktopAds;
	...
	public void verifyAds() {
		...
		Logs.info(getClass(),"Ads found on desktop page " +ads.size());
		if(ads.size()>=2)
		{
	...
	/**
	 * @author AnupamGupta
	 * This method used to validate Advertisement is displayed in Assam desktop
	 */
	public void verifyAssamAds(int wait) {
		int count=0;
		webDriverActions.waitForPageToLoad();
		webDriverActions.waitForSecond(10000);

		if(assamDesktopAds.size()<2) {
			webDriverActions.scrollDown();
			webDriverActions.waitForSecond(1500);
			webDriverActions.scrollToPoint(0, 50);
			webDriverActions.waitForSecond(wait);
		...
		Logs.info(getClass(),"Ads found on page " +assamDesktopAds.size());
		//		webDriverActions.waitForAllElementsVisible(ads, GlobalVariables.sec60);
		if(assamDesktopAds.size()>=2)
		{
			for(int i=0;i<assamDesktopAds.size();i++) {
				if(webDriverActions.isElementDisplayed(assamDesktopAds.get(i))) {
					count++;
				}
			}
			Logs.info(getClass(), "Ads displayed in page is "+count);
